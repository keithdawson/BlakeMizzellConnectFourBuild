        ...
        if (input == ' ' || input == 10) {
            ...
            if (rowAvailable > 0) {
                ...
                if (CountFromPosition(rowAvailable, columnChosen, turn) >= 4) {
                    ...
                    sprintf(output, "%s WINS!", p[turn - 1].name);
                    DrawPrompt(output);
                    napms(1900);
                    Quit();
                }
...
void PlayVsC(){
    ...
    do {
        if (turn == 1){
            ...
            //Make sure player isn't about to win/find best location to play
            for (int i = 1; i < boardXDim; i++){
            ...
            //Check if computer about to win and do it
            for (int i = 1; i < boardXDim; i++){
                rowAvailable=SlotAvailableInRow(i);
...
//Count the number of like pieces in the row, column and xs
...
//Print the location of the piece that you're dropping
